...
public abstract class TestExecutionJob implements Runnable, ExecutionController {
	...
	private static final NBCTestLogger logger = NBCTestLoggingFactory
			.getInstance().getLogger(TestExecutionJob.class);
	...
	public TestExecutionInfo getTestExecutionInfo() {
		lock.lock();
		try {
			TestExecutionInfo info = TestExecutionInfoFactory.getInstance()
					.createTestExecutionInfo(id, status, startTime, stopTime);
			return info;
		} finally {
			lock.unlock();
		}

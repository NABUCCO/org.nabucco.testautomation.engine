...
public class SubEngineInvoker {
	...
	private static final NBCTestLogger logger = NBCTestLoggingFactory
			.getInstance().getLogger(SubEngineInvoker.class);
    ...
    /**
     * Constructs a new instance of a SubEngineInvoker for a certain {@link Metadata} object (the last in the list) and
     * {@link SubEngineActionType}.
     * 
     * @param proxyPool
     ...
     */
    public SubEngineInvoker(Metadata metadata,
            TestContext context, PropertyList properties, SubEngineActionCode action) throws NBCTestConfigurationException {
		...
		if (this.metadata == null || this.metadata.isEmpty()) {
			throw new NBCTestConfigurationException(
					"No Metadata found for invocation");
		}
		if (action == null || action.getCode() == null
				|| action.getCode().getValue() == null) {
			throw new NBCTestConfigurationException(
					"No Action selected for invocation of Metadata '"
		...
		if (invokeMetadata == null || invokeMetadata.getSubEngine() == null
				|| invokeMetadata.getSubEngine().getCode() == null
				|| invokeMetadata.getOperation() == null
				|| invokeMetadata.getOperation().getCode() == null) {
			throw new NBCTestConfigurationException(
					"Metadata not configured properly for execution of SubEngine");
		}
		...
		ProxyEnginePool proxyEnginePool = ProxyPoolFactory.getInstance()
				.getProxyEnginePool();
    ...
    public void invoke() throws NBCTestConfigurationException {
        ...
        try {
            logger.debug("invoking ", method.getName(), " for SubEngineOperationType " +
            		operationType);
            
			Object resultObj = method.invoke(subEngine, operationType, actionType,
					this.metadata, propertyList, context);
            ...
            if (resultObj == null) {
                ...
                throw new NBCTestConfigurationException(
                        "No TestResult received ! received class: "
            ...
            throw new NBCTestConfigurationException("Could not invoke operation '"
                    + method.getName()
                    + "'", ex);
        }
    }

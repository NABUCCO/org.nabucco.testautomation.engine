...
public class TestEngineLauncher {
    ...
    private static final NabuccoLogger logger = NabuccoLoggingFactory.getInstance().getLogger(TestEngineLauncher.class);
    ...
    public void start() throws LaunchingException {
        ...
        try {
            ...
            logger.info("TestExecutionService initialized with thread pool size: "
                    + TestExecutionServiceFactory.THREAD_POOL_SIZE);
            this.testEngine = TestEngineFactory.getInstance().createTestEngine(this.service);
        } catch (TestEngineException ex) {
            throw new LaunchingException("Could not create TestEngine-instance", ex);
        }
    ...
    /*
     * exports the TestEngine interface, so it available via RMI under the defined name in
     * bindingName
     */
    private void exportTestEngine(TestEngine testEngine, int port, String bindingName) throws LaunchingException {
    ...
    public static void main(String[] args) {
        ...
        // Launch TestEngine
        TestEngineLauncher launcher = new TestEngineLauncher(port, bindingName, deployPath);
        try {
            launcher.start();
            Runtime.getRuntime().addShutdownHook(new ShutdownTestEngineHook(launcher));
